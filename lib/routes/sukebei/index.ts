...
const SIZE_UNIT_MULTIPLIERS = { B: 1, KIB: 1024, MIB: 1024 ** 2, GIB: 1024 ** 3, TIB: 1024 ** 4 };

/**
 * Convert a listing size such as `806.3 MiB` into bytes for `enclosure_length`.
 * RSS 2.0 makes `length` mandatory, and readers may discard an enclosure that
 * omits it, so the caller falls back to 0 (the RSS validator's recommended
 * value) when the size cannot be parsed.
 */
function parseSizeToBytes(text: string): number | undefined {
    const match = /([\d.]+)\s*(B|KiB|MiB|GiB|TiB)/i.exec(text);
    if (!match) {
        return undefined;
    }
    const multiplier = SIZE_UNIT_MULTIPLIERS[match[2].toUpperCase() as keyof typeof SIZE_UNIT_MULTIPLIERS];
    return multiplier === undefined ? undefined : Math.round(Number(match[1]) * multiplier);
}

/**
 * Escape a value for HTML text/attribute use: magnet URIs use `&` as their
 * parameter separator, so it has to be entity-encoded to stay valid HTML.
 */
...
const fetchDetail = (item: ListItem) =>
    cache.tryGet(item.link, async (): Promise<DataItem> => {
        ...
        // ── Magnet link ──
        // Prefer the detail page; the listing row carries the same magnet, so
        // fall back to it when the detail page comes back throttled or only
        // partially rendered.
        const magnetLink = $('a[href^="magnet:"]').first().attr('href') || item.magnet || '';
        ...
        // ── Magnet link, appended last so it is easy to copy ──
        // The URI doubles as the link text, while the enclosure still carries
        // the machine readable version for BT clients.
        if (magnetLink) {
            const escapedMagnet = escapeHtml(magnetLink);
            description += `\n<p>Magnet: <a href="${escapedMagnet}">${escapedMagnet}</a></p>`;
        }
    ...
    const items: ListItem[] = $('tr.default')
        ...
        .map((element): ListItem | null => {
            ...
            return {
                ...
                size: parseSizeToBytes(item.find('td:nth-child(4)').text()),
            };
        })
